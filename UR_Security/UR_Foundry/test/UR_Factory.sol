...
/**
 * @title UR_Factory
 * @dev Factory contract for deploying UnRugable tokens backed by USDC
 * Zero launch fee - only gas costs
 */
contract UR_Factory is Ownable {
    ...
    // Mapping from token address to metadata
    ...
    /**
     * @dev Launch a new meme token
     * @param _name Token name
     * @param _symbol Token symbol
     * @param _creatorWallet Creator wallet address
     * @param _imageUri Image URI for the meme
    ...
    function launch(
        ...
        // Deploy new UnrugableToken (USDC backed) from UnRugable.sol
        UnrugableToken newToken = new UnrugableToken(
            _creatorWallet,      // creator (becomes creatorFeeRecipient)
            platformWallet,      // devFeeRecipient (from factory)
        ...
        // Automatically renounce ownership for maximum trust
        newToken.renounceOwnership();
        
        // Store token info (totalSupply is 0 for on-demand minting)
        tokenInfo[token] = TokenInfo({
            ...
            symbol: _symbol,
            imageUri: _imageUri,
            totalSupply: 0, // On-demand minting - starts at 0
